 ...
 * Removes the original cloned lightning text and renders exactly one CSS lightning.
 */
...
add_action( 'wp_footer', function() {
    ...
    @media (max-width:390px){
      ...
      .x13-fixed .x13-mobile-id-card .x13-id-icon::before{font-size:3.95vw!important}
    ...
    (function(){
      function removeOriginalLightning(){
        document.querySelectorAll('.x13-mobile-id-card .x13-id-icon').forEach(function(icon){
          if (icon.textContent !== '') icon.textContent = '';
        });
      }
      document.addEventListener('DOMContentLoaded', function(){
        removeOriginalLightning();
        setTimeout(removeOriginalLightning, 50);
        setTimeout(removeOriginalLightning, 300);
      });
      if (document.body && window.MutationObserver) {
        new MutationObserver(removeOriginalLightning).observe(document.body, { childList:true, subtree:true });
      }
    })();
    </script>
    <?php
